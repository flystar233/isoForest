...
This vignette provides detailed explanations of four threshold determination methods implemented in the isoForest package: Z-score, MAD, Karcher, and MTT.
...
The Karcher method is based on Riemannian geometry, computing the Karcher mean (Fréchet mean or Riemannian center of mass), a geometric center defined on manifolds.

## Formula

**Karcher mean definition:**

The Karcher mean minimizes:

```
f(x) = Σ d²(x_i, x)
```

Where d(·,·) is the distance metric on the manifold (Euclidean distance in Euclidean space).

**In 1D Euclidean space:**

For 1D data (anomaly scores), Karcher mean reduces to weighted arithmetic mean.

**Threshold (complex mode):**

```
1. Normalize: scores_norm = (scores - min) / (max - min)
2. Compute KDE: kde = KDE(scores_norm)
3. Weighted vector: vals = kde ⊙ sort(scores_norm)
4. Karcher mean: karcher_mean = weighted_mean(vals)
5. Denormalize: karcher_mean = karcher_mean × (max - min) + min
6. Threshold = mean(karcher_mean) + k × σ
```

**Threshold (simple mode):**

```
1. Karcher mean: karcher_mean = weighted_mean(scores)
2. Threshold = mean(karcher_mean) + k × σ
```
...
## Workflow (Complex Mode)

1. Normalize scores to [0, 1]
2. Estimate probability density using KDE
3. Element-wise multiplication of KDE and sorted normalized scores
4. Compute weighted average (Karcher mean)
5. Denormalize back to original scale
6. Use SD as scale measure
7. Set threshold = mean(karcher_mean) + k × σ

## Core Concepts

**Why Karcher mean?**

1. Geometric center: True center on manifolds
2. Distribution-aware: KDE weighting considers data distribution
3. Robustness: Resistance to extremes (between mean and median)

**Relationship to other centers:**

```
- Arithmetic mean: minimizes Σ(x_i - c)² (L2 distance)
- Median: minimizes Σ|x_i - c| (L1 distance)
- Karcher mean: minimizes Σd²(x_i, c) (manifold distance)
```

## Advantages

- Geometric intuition from Riemannian geometry
- Distribution-aware (complex mode with KDE)
- Flexible (simple and complex modes)
- Good performance on heavy-tailed distributions

## Limitations

- Computationally complex (complex mode requires KDE)
- Parameter-sensitive (mode and multiplier)
- Less intuitive than MAD
...
- Large datasets (complex mode)

**Example:**
```{r karcher-example, eval=FALSE}
# Complex mode (default, recommended for large datasets)
...
# Adjust sensitivity
result <- set_anomaly_threshold(model, method = "karcher", 
                                karcher_multiplier = 4)  # More conservative
```

**Mode selection:**
- n < 100: use simple mode
- n ≥ 100: use complex mode (default)
...
| Feature | Z-score | MAD | Karcher | MTT |
|---------|---------|-----|---------|-----|
| **Theoretical Basis** | Normal distribution | Median robust statistics | Riemannian geometry | t-distribution test |
...
Medium contamination (1-5%) → MAD or Karcher
...
Extreme anomaly detection → Karcher or MAD
...
methods <- list(
  ...
  karcher = set_anomaly_threshold(model, method = "karcher", karcher_multiplier = 3),
...
3. **Karcher Mean**:
   - Afsari, B. (2011). "Riemannian Lp center of mass: existence, uniqueness, and convexity." *Proceedings of the AMS*.
...
## Q2: How to choose between simple and complex modes for Karcher?

**A**: 
- Simple mode: Small samples (n < 100), direct weighted average
- Complex mode: Large samples (n ≥ 100), uses KDE for distribution density

## Q3: How to set MTT significance level?
...
## Q4: Why is Z-score unsuitable for data with outliers?
...
3. Karcher (simple): Weighted average, O(n)
4. Karcher (complex): Requires KDE, O(n²) or higher
5. MTT: Iterative, O(n × k) where k is iteration count

## Q6: How to validate method effectiveness?
...
methods <- c("zscore", "mad", "karcher", "mtt")
